...
const is_non_empty = <T>(array: readonly T[]): array is NonEmptyArray<T> =>
  array.length > 0;

const last_element = <T>(array: NonEmptyArray<T>): T =>
  array[array.length - 1] as T;
...
/** Log an error (message and optional details) to the console. */
function log_error(message: string, ...args: unknown[]): void {
