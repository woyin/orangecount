...
| Import | `json_api.get_imports` / `get_extract` / `put_add_entries` | `get_imports`, `get_extract`, `save_entries` (import/index.ts, Import.svelte) | `imports` / `extract` / `add_entries` | `internal/web` `handleImport*` (native CSV/bean adapters; Python importers **excluded**) | adapt/partial（上传块已对齐上游 ImportFileUpload 形态：本地读入缓冲区 + 扩展名推断 adapter，`ea13c07`；文件列表/extract 弹窗登记 FD-0001 deviation） |
