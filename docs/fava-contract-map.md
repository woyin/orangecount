...
| `commodities` | `json_api.get_commodities`; Commodities | GET filters | base/quote price series | report price projection | precision/history/unavailable contract + route flow | adopt/partial（base/quote 分组价格表已落地并冒烟；上游 ChartSwitcher+LineChart 折线图未实现） |
