...
| Editor | `json_api.get_source` / `put_source` | `get_source`, `put_source` (editor/index.ts, Editor.svelte) | `source` / `source` (PUT) | `internal/web` `handleEditor` + `handleEditorSave` (atomic write, snapshot reload) | adopt（前端呈现已从只读 textarea 换为移植 CodeMirror 编辑器 `de56b38`：tree-sitter/Lezer 高亮缩进折叠补全，wasm 内联直传字节绕过 fetch，CSP 加 `'wasm-unsafe-eval'`；后端 source 读/写契约不变。deviation：无 format 键绑定、indent 固定 2、无 ruler；File/Edit 菜单与文件树仍余） |
