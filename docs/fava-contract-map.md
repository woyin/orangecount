...
| Query | `json_api.get_query` + `core/query_shell.py` | `get_query` (query/index.ts, Query.svelte) | `query` | `internal/query.Evaluate` (BeanQuery semantics) | adopt/partial（查询图表已落地：恰两列 date+金额结果值嗅探后复用 LineChart + ▼/◀ 开关，`78d4233`；BQL 编辑器已移植 CodeMirror（无 wasm，StreamLanguage），QueryReport 集成，`53bd5d3`；限制：str+Inventory 层级分支不可行（无 dtype）、非上游多查询 shell 历史） |
...
| Editor | `json_api.get_source` / `put_source` | `get_source`, `put_source` (editor/index.ts, Editor.svelte) | `source` / `source` (PUT) | `internal/web` `handleEditor` + `handleEditorSave` (atomic write, snapshot reload) | adopt（前端呈现已从只读 textarea 换为移植 CodeMirror 编辑器 `de56b38`：tree-sitter/Lezer 高亮缩进折叠补全，wasm 内联直传字节绕过 fetch，CSP 加 `'wasm-unsafe-eval'`；File/Edit 菜单 + 文件树已落地 `3b48ebe`：File 文件树从适配器 paths 本地建树、Edit 菜单 Toggle Comment/folds；后端 source 读/写契约不变。deviation：无 format 键绑定/Align Amounts、indent 固定 2、无 ruler、无 insert-entry options 菜单） |
| Import | `json_api.get_imports` / `get_extract` / `put_add_entries` | `get_imports`, `get_extract`, `save_entries` (import/index.ts, Import.svelte) | `imports` / `extract` / `add_entries` | `internal/web` `handleImport*` (native CSV/bean adapters; Python importers **excluded**) | adapt/partial（上传块已对齐上游 ImportFileUpload 形态：本地读入缓冲区 + 扩展名推断 adapter，`ea13c07`；文件列表/extract 弹窗登记 FD-0001 deviation，已批准 approved 2026-08-08） |
...
| `context` | `json_api.get_context`; Context modal | GET `entry_hash` | entry plus balances before/after | report/context projection | hash lookup + modal flow | adapt/partial（OC 私有路由 `entry-context` 合并 context+source slice 只读投影；balances before/after 已落地 `b833415`，Transaction/Balance 按账户×货币分组展示） |
...
| `upload_import_file` | `json_api.put_upload_import_file`; Import | PUT multipart | local candidate identifier/status | native import service | containment/type/size/error + browser flow | adapt/deviated（OC 无 import 目录：上传块改为浏览器本地 FileReader 读入导入缓冲区，无服务端暂存路由，`ea13c07`） |
...
| `documents` | `json_api.get_documents`; Documents | GET filters | serialized Documents | report + Document Roots | grouping/filter/unsafe-state contract + route flow | adapt/partial（行选中 + `/documents/` 内联预览 `92ccb40`、账户树侧栏 `30ca6f3`、move/rename 模态与私有 `move-document` 路由 `38f2618` 均已落地；上游 unsafe-state 提示与分页未实现） |
...
| `add_document` | `json_api.put_add_document`; DocumentUpload | PUT multipart | status/path identifier | Reviewed document workflow | containment/type/partial failure + flow | adapt/partial（OC 私有 POST `document` 路由已接线：同源校验 + 账户/目录校验 + basename 净化 + 拒绝覆盖，DocumentUploadModal 走该路由，`339b63e`；droptarget 覆盖补齐至上游四处——账户页标题/documents 账户表/journal 行/账户树单元格，`44e6476`；entry hash 附件元数据与 uri-list 链接拖放未实现） |
