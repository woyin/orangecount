...
| T2 | R-ACCOUNT | 账户详情页过薄 | 标题含 `(Last entry: date)` 与科目层级面包屑；Balance/Changes 切换；Account Balance/Changes(monthly)/Balances(monthly) 三区块；账户图 + 货币圆点；Account Journal 带徽章 | 面包屑与 Last entry 指示器已实现（`88d90b9`/`eefbf83`），Journal 带 change 列（`ae4eedc`）；上游式 `?r=changes\|balances` 三区块切换 + 区间表已落地（`24ef4ca`：AccountIntervals 按账户子树聚合逐区间变动与累计余额，含安静区间补齐、时间过滤、月度/季度/年度）；账户图已落地（`8da2359`：账户路由已返回 accountChart 数据，前端 AccountReport 解析并复用 ReportChart 组件渲染于 Balance 区块上方，冒烟验证 /account/Assets 渲染 report-line-chart + "Account balance · USD" 标题）；up-to-date 状态指示已落地（`c9ed0a6`：适配器按账户最近条目类型投影 uptodate_status（green=通过的 balance 断言、yellow=交易；red 因 valid-only 不可达，FD-0002），account_details 经 bootstrap→state→ReportOutlet 贯通，账户页面包屑旁渲染状态圆点，冒烟验证 Wallet01 green/Index05 yellow）（限制：IntervalTreeTable 的账户子树逐列形态与图表货币圆点（图例）未实现；FQL 文本过滤不作用于区间聚合行） | 前端 + 适配器 |
