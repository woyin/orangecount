...
| T2 | R-ACCOUNT | 账户详情页过薄 | 标题含 `(Last entry: date)` 与科目层级面包屑；Balance/Changes 切换；Account Balance/Changes(monthly)/Balances(monthly) 三区块；账户图 + 货币圆点；Account Journal 带徽章 | 面包屑与 Last entry 指示器已实现（`88d90b9`/`eefbf83`），Journal 带 change 列（`ae4eedc`）；上游式 `?r=changes\|balances` 三区块切换 + 区间表已落地（`24ef4ca`：AccountIntervals 按账户子树聚合逐区间变动与累计余额，含安静区间补齐、时间过滤、月度/季度/年度）；账户图已落地（`8da2359`：账户路由已返回 accountChart 数据，前端 AccountReport 解析并复用 ReportChart 组件渲染于 Balance 区块上方，冒烟验证 /account/Assets 渲染 report-line-chart + "Account balance · USD" 标题）；up-to-date 状态指示已落地（`c9ed0a6`：适配器按账户最近条目类型投影 uptodate_status（green=通过的 balance 断言、yellow=交易；red 因 valid-only 不可达，FD-0002），account_details 经 bootstrap→state→ReportOutlet 贯通，账户页面包屑旁渲染状态圆点，冒烟验证 Wallet01 green/Index05 yellow）（限制：IntervalTreeTable 的账户子树逐列形态未实现；FQL 文本过滤已作用于区间聚合行（`8d50cfd`：AccountIntervals 对每个 posting 应用 FQL，chartPosting 携带 payee/narration/tags/links/flag，冒烟 payee:Synthetic 9→4 行）；图表货币圆点图例已随 ReportChart 在案——`.legend i` 圆形色点 + 可点击 per-currency 切换，`32cceb0`） | 前端 + 适配器 |
...
| # | Manifest | 差距 | Fava 行为 | 移植版现状 | 根因层 |
| --- | --- | --- | --- | --- | --- |
| H1 | R-EDITOR/R-QUERY | CodeMirror 未移植（上游 19 文件 + tree-sitter wasm） | 语法高亮、行号、折叠、补全、snippets、File/Edit 菜单、文件树 | CodeMirror 编辑器已落地（`de56b38`，见 H1-editor）：上游 `frontend/src/codemirror` 单元近原样移植到 `web/src/fava/codemirror/`（tree-sitter+Lezer 高亮/折叠/缩进/补全/snippets），EditorReport 以 CodeMirror 取代只读 textarea——实际为可编辑损益（上游编辑器可编辑 + 保存，非纯展示），初始 `loadFile` 载入、切换文件 `replace_contents`、`updateListener` 同步内容，eadapter bootstrap 喂 completion data；冒烟验证 `.cm-editor` 渲染、gutter 行号、27 个高亮 span、输入字符生效、Save 流程（界面 "File saved."、磁盘写入 + `.orangecount.bak` 备份、refresh 后 loadFile 恢复）。wasm 以 base64 data URL 内联，前端解码后直传 `TSParser.init`/`Language.load` 字节绕过其 fetch 路径（streaming 无法加载 data: URL）；CSP `script-src` 放宽加 `'wasm-unsafe-eval'`（only wasm 编译，不引入任意 eval）。三项 deviation：① 无 beancount-format 键绑定（OC 无 format 端点）；② indent 固定 2（bootstrap 无 indent 选项）；③ ruler 未启用（bootstrap 无 currency_column 选项）。File/Edit 菜单与文件树已落地（`3b48ebe`，见 H1-menu）：上游 `reports/editor` 的 AppMenu 族 + Sources 递归文件树 + lib/sources+tree+paths 移植为 legacy Svelte，EditorMenu 挂在 EditorReport 上——File 菜单从适配器 paths 本地建树（editor_sources store，替代上游 sources API）、点叶节点切换文件；Edit 菜单 Toggle Comment / Open/Close all folds（复用 beancount.ts 的 toggleComment/foldAll/unfoldAll）。deviation：Align Amounts（beancount_format，无 format 端点）与 insert-entry 选项菜单（无 insert_entry store）省略。BQL 编辑器已随 M1 落地（`53bd5d3`，见 M1-bql-editor） | 前端组件缺失（H1 编辑器/菜单/文件树已落地，收尾中） |
...
| H5 | R-JOURNAL | Journal 交互层不完整 | 全量条目类型徽章（含 Custom/B/Metadata/Postings）、排序与列菜单、点击条目→Context、URL 同步筛选、拖拽上传文档 | 核心徽章组与展开已现；点击条目→Context 已落地（`68ddcfa`：行尾 ⋮ 链接 + `#context-<hash>` 模态 + entry-context 私有路由，位置派生 entry_hash，只读源码切片；before/after 余额已落地 `b833415`——Transaction/Balance 条目在模态中显示受影响账户在条目前/后的余额，分组按账户×货币，镜像上游 get_context balances_before/after；限制：CodeMirror 可编辑切片属 H1）；表头排序已落地（`f7d57ab`：Date/F/Payee-Narration 三列，同上游 `[列,向]` localStorage 持久化、默认 date desc、data-order 箭头与切换语义，复用已移植 Sorter）。拖拽上传文档已随 H2 M-DOCUMENT 落地（`339b63e`：账户页标题 droptarget；journal 行与账户树单元格的 droptarget 后经 H2-droptarget-extend 补齐，`44e6476`）。徽章覆盖复核已落地（`4bafc76`：适配器为全部指令类型投影条目元数据——此前仅 transaction——journal 行渲染条目与过账级 metadata-indicator 徽章（key[:2]，title `key: value`）、过账 flag 类（flag_to_type）、过账元数据 dl、linked/discovered 行类与 D/L 芯片（`d d`/`d l`，show-document 的子级，默认激活集与上游 default_journal_show 同构）；冒烟验证 au/re/cl 徽章、linked/discovered 行类、过账 dl 展开与 Metadata 芯片切换）。稠密夹具复比勾销已落地（`6a3233b`：fixturegen 补 `#linked`/`#discovered` 文档交易，/journal 冒烟确认 linked/discovered 行类（`document … linked/discovered`）与 D/L 芯片 aria-pressed toggle 各自隐藏对应行、全指令族 entry-type 行类与 flag_to_type 分类、metadata-indicator 徽章与 Metadata 芯片 toggle 均在稠密夹具下复现）。限制：B(budget) 芯片未加——OC 无上游 budget custom 指令形态、适配器不产生 budget 标记，且 ADR-0017 明确第一版不引入预算，登记 FD-0003 已批准（approved）；custom 值按 dtype 渲染已落地（`ad4aebb`：适配器从 ledger ValueKind 投影 typed custom values，journal 按 dtype 渲染——account 链接、amount 格式化、string 带引号、bool/date 字面、number 原样；冒烟验证 risk-score 的 `3.5 USD`/`True` 与 review-state 的 `"pending"`/`"synthetic"`）；balance diff_amount pending 展示不可达——OC serve（`cmd/orangecount/main.go:181`）硬性拒绝含 error 诊断的 ledger（会计语义权威：只服务 valid ledger），故 balance 断言失败（diff_amount 的触发条件）永不进入运行时，登记 FD-0002 已批准（approved）；document→statement 元数据附件链接已落地（`7cc48ca`：journal metadata dl 中 `document` 前缀 key 渲染为指向 `/documents/<value>` 的 filename 链接，剥引号；上游指向 statement 视图，OC 无该路由，链接到文档文件作为替代，statement 完整视图登记为 H1 边界，`/documents/` 链接替代已获产品 owner 接受） | 前端组件 |
...
| M2 | R-IMPORT | Import 为 OC 原创表单 | 上传块已对齐（`ea13c07`，见 M2-import-upload）：上游 ImportFileUpload 形态的文件选择器落地（本地读入导入缓冲区、按扩展名推断 adapter）；文件列表 + 逐条目 extract/review 弹窗已登记并批准为 deviation（FD-0001 approved，见 M2-extract；上游依赖 Python importer 智能，OC 已批准排除 Python importer，Go 原生无法等价复刻 importer 的文件识别/逐条提取/账号推断）；Source path/Adapter/Target + 粘贴缓冲区保留为 OC 扩展 |
...
- entry context 的 before/after 余额（已落地 `b833415`，见 H5 行）
...
> （provenance 登记，上游 hash 按重建计算已注明）；before/after 余额
> 后已落地（`b833415`：适配器遍历发布条目流累加涉及账户的 postings
> 到目标条目前为 before、再加目标自身 postings 为 after，仅
> Transaction/Balance 承载，按账户×货币分组，前端模态以 dl 展示于
> 源码块上方；冒烟验证买入交易 Index05 前后 88.22→97.22 SHARES）；
> CodeMirror 可编辑切片仍属 H1。冒烟验证 400 条 ⋮ 链接、
...
> 页同构代码，后者已验证）；line/area 模式切换（lineChartMode）已落地
> （`93c3afc`：commodities 价格图加 Line/Area 切换，area 模式闭合到零基线
> 填充 + 值域含零，localStorage 持久化；默认 line 无 area-path，切换按钮
> 渲染验证，交互冒烟受 agent-browser os-error-35 挂死限制），
...
> 边界），已由产品 owner 批准为 approved（2026-08-08）；OC 保留本地缓冲
> 单文件 + 通用 adapter 的 preview→commit 工作流。不试图用 Go 重写通用
> 对账单识别（过宽、识别正确性无边界）。
...
> 前置步。随后 CodeMirror 编辑器集成已落地，`de56b38`（见 H1-editor）；
> 查询编辑器 BQL 集成已落地，`53bd5d3`（见 M1-bql-editor）。
> H1-menu（编辑器 File/Edit 菜单与文件树落地：上游
> frontend/src/reports/editor 的 AppMenu/AppMenuItem/AppMenuSubItem/Key/
> Sources/EditorMenu 移植为 OC legacy Svelte，配套 lib/sources.ts+
> lib/tree.ts+lib/paths.ts 移植（tree.ts 剪掉 stratifyAccounts 及
> account.ts 依赖）；EditorReport 挂 EditorMenu，File 菜单以
> editor_sources store 承接适配器扁平 paths 本地建树（替代上游
> sources API），点叶节点走 openFile 切换文件；Edit 菜单在编辑器就绪后
> 显示 Toggle Comment / Open/Close all folds（beancount.ts 补 re-export
> toggleComment/foldAll/unfoldAll）。冒烟验证：File 菜单渲染
> main/accounts/activity/directives 四叶、点 accounts.bean 切换编辑器
> 内容、Edit 菜单 Toggle Comment 生效（`;` 注释 toggle）。限制/偏差：
...
| 项 | 内容 | 完成判据 |
| --- | --- | --- |
| P0-1 | 稠密夹具缺口复核 | 已复核并在本会话补齐（`6a3233b`：扩展 fixturegen 加 `#linked`/`#discovered` 文档交易，覆盖 journal linked/discovered 复现所需；其余差距状态经稠密夹具复比已在 T1/H5-recheck 勾销） |
| P0-2 | 候选截图扩面 + 产品 owner 批准 | 延后为独立后续 QA 流程（产品 owner 决策 2026-08-08）：候选捕获扩面 + `testdata/visual-baselines/` 批准不在此推进，不阻塞功能完成 |
| P0-3 | 每条可复现差距在合成账本上落回归证据 | 持续质量流程：差距条目与测试/截图证据挂钩；本会话已以稠密夹具复比 + 冒烟证据覆盖主要差距 |
...
| D3 | Import 页 OC 原创表单 | **已决策并批准（2026-08-08）**：上传块对齐上游 ImportFileUpload 形态（本地缓冲）；文件列表 + 逐条目 extract/review 因依赖 Python importer 生态（已排除）登记为 FD-0001 deviation 且已批准（approved）；OC 保留 Source path/Adapter/Target + 粘贴缓冲区的本地 preview→commit 工作流 |
...
本清单中的差距（T1–T4 / H1–H7 / M1–M6 / L1–L5）已全部关闭：或已实现并验证，
或已登记并批准为偏差（FD-0001/0002/0003/0004/0005 均获产品 owner 批准为
approved）。剩余仅为已登记的实现层差异 / 划界限制 / 计划项，不构成遗留半成品：

| 类别 | 条目 | 状态 |
| --- | --- | --- |
| 实现层差异 | IntervalTreeTable 账户子树逐列形态（每区间账户树 vs OC 扁平表） | 已登记（T2） |
| H1 边界 | CodeMirror 可编辑切片（context 模态只读源码切片） | 已登记（H5） |
